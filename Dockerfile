...
RUN set -xe && \
  ...
  if [ $APP_PLUGIN_MSSQL = 1 ]; then \
     curl -fSL --connect-timeout 10 "${MSSQL_JDBC_BASEURL}/sqljdbc_${MSSQL_JDBC_VER}.0_enu.tar.gz" | tar xz --wildcards --strip 2 -C "${CATALINA_HOME}/lib/" "*/enu/jars/mssql-jdbc-${MSSQL_JDBC_VER}.jre11.jar" \
  ;fi && \
  \
